...
object Cursor {
  ...
  final class Completed(val path: ColumnPath) extends Cursor {
    ...
    override def advance[FieldName <: String & Singleton : ValueOf]: Option[Cursor] = None
  }
...
private class SkippingCursor private (val path: ColumnPath, toSkip: Set[ColumnPath]) extends Cursor with Cursor.Active {
  ...
  override def advance[FieldName <: String & Singleton: ValueOf]: Option[Cursor] = {
    val newPath = path.appendElement(summon[ValueOf[FieldName]].value)
